...
const sectionColorKey = (label = "") => {
  ...
  for (const k of Object.keys(SEC_C)) if (t.startsWith(k.toLowerCase())) return k;
  return t || "Section";
};
...
function LyricEditor({ text, setText, onContextMenu, sectionColors = SEC_C, onCaretLine, onKeyDown, apiRef, ghosts, onGhostApply, onGhostDelete, onAddVariant, theme = THEME_DEFAULT }) {
  ...
  // Dragging a [Section] header moves the whole section (header through the line before the next header),
  // and the drop point snaps to section boundaries so sections are never split.
  const [lineDrag, setLineDrag] = useState(null); // { from, end, section, to } — [from,end) is the block, `to` an insert-before index
  const lineDragRef = useRef(null), textRef = useRef(text), autoScrollRef = useRef(0);
  textRef.current = text;
  const curLines = () => textRef.current.split("\n");
  const sectionBounds = (arr) => { const b = [0]; arr.forEach((l, i) => { if (i > 0 && getSecLabel(l)) b.push(i); }); b.push(arr.length); return b; };
  const targetFromY = (clientY, drag) => {
    if (!gut.current) return 0;
    const arr = curLines();
    const rect = gut.current.getBoundingClientRect();
    const y = clientY - rect.top - 16 + (ta.current?.scrollTop || 0);
    const raw = Math.max(0, Math.min(arr.length, Math.round(y / LH)));
    if (!drag?.section) return raw;
  ...
  const onLinePointerDown = (i) => (e) => {
    ...
    e.preventDefault();
    const arr = curLines();
    const section = !!getSecLabel(arr[i] || "");
    let end = i + 1;
    if (section) { while (end < arr.length && !getSecLabel(arr[end])) end += 1; }
    const start = { from: i, end, section, to: i };
    lineDragRef.current = start; setLineDrag(start);
    let lastY = e.clientY;
    const update = (clientY) => {
      const cur = lineDragRef.current; if (!cur) return;
      const to = targetFromY(clientY, cur);
...
function EditableName({ name, onSave, onClick, style: { fontSize: fs = 11, ...rest } = {} }) {
  const [editing, setEditing] = useState(false);
  const [v, setV] = useState(name);
  const clickTimer = useRef(null);
  if (editing) return <input autoFocus value={v} onClick={(e) => e.stopPropagation()} onChange={(e) => setV(e.target.value)} onBlur={() => { onSave(v.trim() || name); setEditing(false); }} onKeyDown={(e) => { if (e.key === "Enter") { onSave(v.trim() || name); setEditing(false); } if (e.key === "Escape") setEditing(false); }} style={{ background: "#0a0a0a", border: "1px solid #4af0a040", borderRadius: 3, padding: "1px 4px", fontSize: fs, color: "#c8ccd8", outline: "none", fontFamily: ff, width: "100%", boxSizing: "border-box", ...rest }} />;
  return <span onClick={(e) => { if (!onClick) return; e.stopPropagation(); if (clickTimer.current) clearTimeout(clickTimer.current); clickTimer.current = setTimeout(() => { onClick(e); clickTimer.current = null; }, 180); }} onDoubleClick={(e) => { e.stopPropagation(); if (clickTimer.current) clearTimeout(clickTimer.current); setV(name); setEditing(true); }} style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", cursor: onClick ? "pointer" : "default", fontSize: fs, ...rest }} title="ダブルクリックで名前変更">{name}</span>;
}
