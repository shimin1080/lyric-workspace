...
export default function LyricWorkspace() {
  ...
  useEffect(() => {
    remoteRef.current = async (data) => {
      ...
      // Merge audio lists (don't replace, to avoid losing local tracks)
      if (data.audioLib) setAudioLib(data.audioLib);
      if (data.recLib) setRecLib(data.recLib);
      // Download any new audio files from cloud
  ...
  const doSave = useCallback((o = {}) => {
    ...
    saveTimerRef.current = setTimeout(async () => {
      await _saveData(S_KEY, { projects: o.projects || projects, lyrics: o.lyrics || lyrics, cards: o.cards || cards, activeProj: o.activeProj || activeProj, audioLib: o.audioLib || audioLib, recLib: o.recLib || recLib, memo: o.memo || memo, trash: o.trash || trash, projectList: o.projectList || projectList });
      // Push to cloud if logged in
      if (user) { push({ projects: o.projects || projects, lyrics: o.lyrics || lyrics, cards: o.cards || cards, activeProj: o.activeProj || activeProj, memo: o.memo || memo, audioLib: o.audioLib || audioLib, recLib: o.recLib || recLib, trash: o.trash || trash, projectList: o.projectList || projectList }); }
      setSaveStatus("saved"); setTimeout(() => setSaveStatus("idle"), 2000);
    }, 800);
  }, [projects, lyrics, cards, activeProj, audioLib, recLib, memo, trash, projectList, user, push]);
