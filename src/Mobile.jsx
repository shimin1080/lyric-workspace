...
export default function MobileApp(){
  ...
  /* ── Save ── */
  const doSave=useCallback((o={})=>{if(saveTimer.current)clearTimeout(saveTimer.current);setSaveStatus("saving");saveTimer.current=setTimeout(async()=>{const d={projects:o.projects||projects,lyrics:o.lyrics||lyrics,cards:o.cards||cards,activeProj:o.activeProj||activeProj,audioLib:o.audioLib||audioLib,recLib:o.recLib||recLib,memo:o.memo||memo,trash:o.trash||trash,projectList:o.projectList||projectList};await saveAppData(d);if(user){push(d);}setSaveStatus("saved");setTimeout(()=>setSaveStatus("idle"),1500);},800);},[projects,lyrics,cards,activeProj,audioLib,recLib,memo,trash,projectList,user,push]);
