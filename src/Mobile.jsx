...
const PanelIcon=({on,size=16})=>(<svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" style={{flexShrink:0}}><rect x="3" y="4" width="18" height="16" rx="2"/><rect x="3" y="4" width="18" height="5" rx="2" fill="currentColor" style={{opacity:on?0.9:0,transition:"opacity 220ms var(--lw-motion)"}}/><line x1="3" y1="9" x2="21" y2="9" style={{opacity:on?0:0.6,transition:"opacity 220ms var(--lw-motion)"}}/></svg>);
...
const normalizeThemeStore=(v,activeProj)=>{if(!v||typeof v!=="object")return{};if(typeof v.text==="string"||typeof v.bg==="string")return activeProj?{[activeProj]:normalizeTheme(v)}:{};return v;};
// Section variants (shared shape with App.jsx): sectionVariants["project:take"][label] = { variants:[{id,text}], activeId }
...
function listSections(lines){const out=[];lines.forEach((l,i)=>{const lb=getSecLabel(l);if(lb)out.push({label:lb.trim(),start:i,end:lines.length});});out.forEach((sec,k)=>{if(k+1<out.length)sec.end=out[k+1].start;const body=lines.slice(sec.start+1,sec.end);let trail=0;while(trail<body.length&&body[body.length-1-trail].trim()==="")trail+=1;sec.trail=trail;sec.body=body.slice(0,body.length-trail).join("\n");});return out;}
function replaceSectionBody(lines,label,body){const secs=listSections(lines).filter(sec=>sec.label===label);if(!secs.length)return lines;const out=[];let cursor=0;for(const sec of secs){out.push(...lines.slice(cursor,sec.start+1));if(body!=="")out.push(...body.split("\n"));for(let i=0;i<sec.trail;i++)out.push("");cursor=sec.end;}out.push(...lines.slice(cursor));return out;}
...
function MobileColorPopover({anchor,value,defaultValue,onChange,onClose,title}){
  const ref=useRef(null);
  useEffect(()=>{const onDown=e=>{if(ref.current?.contains(e.target))return;onClose();};const onKey=e=>{if(e.key==="Escape")onClose();};document.addEventListener("pointerdown",onDown,true);window.addEventListener("keydown",onKey);return()=>{document.removeEventListener("pointerdown",onDown,true);window.removeEventListener("keydown",onKey);};},[onClose]);
  const width=220;const left=Math.max(8,Math.min(anchor.left,window.innerWidth-width-8));
  const isDefault=String(value).toLowerCase()===String(defaultValue).toLowerCase();
  return (<div ref={ref} className="lw-motion-dropdown" style={{position:"fixed",left,top:anchor.bottom+6,width,zIndex:2400,background:"#111116",border:"1px solid #4a4e5e",borderRadius:10,boxShadow:"0 20px 40px rgba(0,0,0,0.5)",padding:12,fontFamily:ff}}>
    <div style={{fontSize:11,color:"#7a7e8e",marginBottom:8,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{title}</div>
    <div style={{display:"flex",alignItems:"center",gap:8}}>
      <label style={{position:"relative",width:40,height:32,borderRadius:6,border:"1px solid #3a3a4a",background:value,flexShrink:0}}><input type="color" value={value} onChange={e=>onChange(e.target.value)} style={{position:"absolute",inset:0,width:"100%",height:"100%",opacity:0}}/></label>
      <input value={value} onChange={e=>{const v=e.target.value.trim();if(/^#[0-9a-fA-F]{6}$/.test(v))onChange(v);}} spellCheck={false} style={{flex:1,minWidth:0,background:"#0a0a0d",border:"1px solid #3a3a4a",borderRadius:6,padding:"7px 8px",fontSize:13,color:"#c8ccd8",outline:"none",fontFamily:mf}}/>
    </div>
    <button disabled={isDefault} onClick={()=>onChange(defaultValue)} style={{marginTop:10,width:"100%",padding:"8px 0",borderRadius:6,border:"1px solid #3a3a4a",background:"transparent",color:isDefault?"#4a4e5e":"#c8ccd8",fontSize:12,fontFamily:ff}}>初期化</button>
  </div>);
}
...
export default function MobileApp(){
  ...
  const[takeMenu,setTakeMenu]=useState(null); // { left, top, width }
  const takeMenuRef=useRef(null);
  useEffect(()=>{if(!takeMenu)return;const onDown=e=>{if(takeMenuRef.current?.contains(e.target)||e.target.closest?.("[data-take-anchor]"))return;setTakeMenu(null);};const onKey=e=>{if(e.key==="Escape")setTakeMenu(null);};document.addEventListener("pointerdown",onDown,true);window.addEventListener("keydown",onKey);return()=>{document.removeEventListener("pointerdown",onDown,true);window.removeEventListener("keydown",onKey);};},[takeMenu]);
  ...
  useEffect(()=>{
    remoteRef.current=async(data)=>{const incomingTime=syncTime(data);const hasUnsyncedLocal=localUpdatedAtRef.current>localLastSyncedAtRef.current;if(hasUnsyncedLocal)return;if(incomingTime&&localUpdatedAtRef.current&&incomingTime<localUpdatedAtRef.current)return;if(data.projects)setProjects(data.projects);if(data.lyrics)setLyrics(data.lyrics);if(data.cards)setCards(data.cards);if(data.activeProj)setActiveProj(data.activeProj);if(data.memo)setMemo(data.memo);if(data.drafts)setDrafts(data.drafts);if(data.activeDrafts)setActiveDrafts(data.activeDrafts);if(data.sectionColors)setSectionColors(normalizeSectionColors(data.sectionColors));if(data.sectionVariants)setSectionVariants(data.sectionVariants);if(data.editorTheme)setEditorTheme(normalizeThemeStore(data.editorTheme,data.activeProj));if(data.trash)setTrash(data.trash);if(data.projectList)setProjectList(data.projectList);if(data.projectFolders)setProjectFolders(data.projectFolders);if(data.audioLib)setAudioLib(data.audioLib);if(data.recLib)setRecLib(data.recLib);const merged={...stateRef.current,...data};localUpdatedAtRef.current=syncTime(merged)||Date.now();localLastSyncedAtRef.current=localUpdatedAtRef.current;merged.__updatedAt=localUpdatedAtRef.current;merged.__lastSyncedAt=localLastSyncedAtRef.current;stateRef.current=merged;await saveAppData(merged);if(user&&(data.audioLib||data.recLib)){await syncAudioOnLogin(user.id,data.audioLib||[],data.recLib||[],loadAudio,saveAudio,S_AP,S_RC,audioCache);}};
  ...
  const commitText=(t,storeOverride)=>{const base=drafts[activeProj]?.length?drafts[activeProj]:draftList;const list=base.map(d=>d.id===activeDraft.id?{...d,text:t}:d);const nd={...drafts,[activeProj]:list};const nad={...activeDrafts,[activeProj]:activeDraft.id};const nl={...lyrics,[activeProj]:t};setDrafts(nd);setActiveDrafts(nad);setLyrics(nl);const o={drafts:nd,activeDrafts:nad,lyrics:nl};if(storeOverride){const nsv={...sectionVariants,[varScope]:storeOverride};setSectionVariants(nsv);o.sectionVariants=nsv;}doSave(o);};
  // Editing: the edited occurrence (under the caret, else the first that differs) becomes the active variant's text
  const setCurText=t=>{let next=null;if(Object.keys(varStore).length){const secs=listSections(t.split("\n"));const el=mobileTextRef.current;const cl=el?el.value.substring(0,el.selectionStart).split("\n").length-1:mobileCurrentLine;for(const[label,entry]of Object.entries(varStore)){const active=entry.variants.find(v=>v.id===entry.activeId);if(!active)continue;const occ=secs.filter(sec=>sec.label===label&&sec.body!==active.text);const changed=occ.find(sec=>cl>=sec.start&&cl<sec.end)||occ[0];if(changed){next=next||{...varStore};next[label]={...entry,variants:entry.variants.map(v=>v.id===active.id?{...v,text:changed.body}:v)};}}}commitText(t,next);};
  const caretSection=sectionAtLine(curLines,mobileCurrentLine);
  ...
  const focusSection=label=>{const sec=listSections(curLines).find(x=>x.label===label);if(!sec)return;setTimeout(()=>{const el=mobileTextRef.current;if(!el)return;const st=el.scrollTop;const pos=el.value.split("\n").slice(0,sec.start).reduce((n,l)=>n+l.length+1,0);el.selectionStart=el.selectionEnd=pos;el.scrollTop=st;syncMobileEditor(el.value);},0);};
  ...
  const sections=[];curText.split("\n").forEach(l=>{const lb=getSecLabel(l);if(lb)sections.push({label:lb,key:sectionColorKey(lb),color:getSecColor(l,sectionColors)});});
  const mobileLines=curText.split("\n");
  const mobileSecMap=buildSecMap(mobileLines,sectionColors);
  const mobileLineHeight=32;
  let mobileSectionLine=0;

  useEffect(()=>{projectDragRef.current=projectDrag;},[projectDrag]);
  // Gutter drag: move one line, or a whole [Section] block when its header is grabbed (snaps to section boundaries)
  const gutterLines=()=>(mobileTextRef.current?.value??curText).split("\n");
  const sectionBounds=arr=>{const b=[0];arr.forEach((l,i)=>{if(i>0&&getSecLabel(l))b.push(i);});b.push(arr.length);return b;};
  const gutterTargetFromY=(clientY,drag)=>{const g=mobileGutterRef.current;const el=mobileTextRef.current;if(!g||!el)return 0;const arr=gutterLines();const r=g.getBoundingClientRect();const y=clientY-r.top-16+el.scrollTop;const raw=Math.max(0,Math.min(arr.length,Math.round(y/mobileLineHeight)));if(!drag?.section)return raw;return sectionBounds(arr).reduce((best,b)=>Math.abs(b-raw)<Math.abs(best-raw)?b:best,0);};
  const moveGutterBlock=({from,end,to})=>{const arr=gutterLines();if(from<0||end>arr.length||(to>=from&&to<=end))return;const block=arr.slice(from,end);const rest=[...arr.slice(0,from),...arr.slice(end)];const ins=to>from?to-(end-from):to;rest.splice(ins,0,...block);const t=rest.join("\n");setCurText(t);const pos=rest.slice(0,ins).reduce((n,l)=>n+l.length+1,0);const st=mobileTextRef.current?.scrollTop||0;setTimeout(()=>{const el=mobileTextRef.current;if(!el)return;el.selectionStart=el.selectionEnd=pos;el.scrollTop=st;syncMobileEditor(el.value);},0);};
  const onGutterPointerDown=i=>e=>{if(e.pointerType==="mouse"&&e.button!==0)return;e.preventDefault();const arr=gutterLines();const section=!!getSecLabel(arr[i]||"");let end=i+1;if(section){while(end<arr.length&&!getSecLabel(arr[end]))end+=1;}const start={from:i,end,section,to:i};lineDragRef.current=start;setLineDrag(start);let lastY=e.clientY;const update=y=>{const cur=lineDragRef.current;if(!cur)return;const to=gutterTargetFromY(y,cur);if(to!==cur.to){lineDragRef.current={...cur,to};setLineDrag(lineDragRef.current);}};const tick=()=>{const el=mobileTextRef.current;if(!el||!lineDragRef.current)return;const r=el.getBoundingClientRect();const edge=40;let dy=0;if(lastY<r.top+edge)dy=-Math.ceil((r.top+edge-lastY)/4);else if(lastY>r.bottom-edge)dy=Math.ceil((lastY-(r.bottom-edge))/4);if(dy){el.scrollTop+=dy;setMobileScrollTop(el.scrollTop);update(lastY);}lineDragScrollRef.current=requestAnimationFrame(tick);};const move=ev=>{lastY=ev.clientY;update(ev.clientY);};const up=()=>{window.removeEventListener("pointermove",move);window.removeEventListener("pointerup",up);window.removeEventListener("pointercancel",up);cancelAnimationFrame(lineDragScrollRef.current);const cur=lineDragRef.current;lineDragRef.current=null;setLineDrag(null);if(cur)moveGutterBlock(cur);};window.addEventListener("pointermove",move);window.addEventListener("pointerup",up);window.addEventListener("pointercancel",up);lineDragScrollRef.current=requestAnimationFrame(tick);};
  const gutterDropTop=lineDrag?16+lineDrag.to*mobileLineHeight-mobileScrollTop:0;
  const showGutterDrop=lineDrag&&!(lineDrag.to>=lineDrag.from&&lineDrag.to<=lineDrag.end);
  const syncMobileEditor=(nextText=curText)=>{const el=mobileTextRef.current;if(!el)return;setMobileScrollTop(el.scrollTop);const pos=el.selectionStart||0;const before=nextText.substring(0,pos);const parts=before.split("\n");const line=parts.length-1;const lineText=parts.at(-1)||"";setMobileCurrentLine(line);let tw=lineText.length*9.65;try{const cv=document.createElement("canvas");const ctx=cv.getContext("2d");ctx.font="16px 'Courier New', 'JetBrains Mono', ui-monospace, Menlo, monospace";tw=ctx.measureText(lineText).width;}catch(e){}setMobileCaret({top:16+line*mobileLineHeight-el.scrollTop,left:53+tw-el.scrollLeft,visible:document.activeElement===el});};
  ...
  const switchProject=id=>{const list=projectDrafts(drafts,id,lyrics[id]||"");const draftId=activeDrafts[id]||list[0]?.id;const draft=list.find(d=>d.id===draftId)||list[0];const nl={...lyrics,[id]:draft?.text||""};setActiveProj(id);setLyrics(nl);setTagFilter("all");closeProjectPicker();doSave({activeProj:id,lyrics:nl});};
  const addProject=()=>{if(!newProjTitle.trim())return;const id="proj_"+Date.now(),initialDraft=makeDraft(0,""),np=[...projects,{id,title:newProjTitle.trim()}];const nl={...lyrics,[id]:""};const nd={...drafts,[id]:[initialDraft]};const nad={...activeDrafts,[id]:initialDraft.id};setProjects(np);setLyrics(nl);setDrafts(nd);setActiveDrafts(nad);setActiveProj(id);setShowNewProj(false);setNewProjTitle("");doSave({projects:np,lyrics:nl,drafts:nd,activeDrafts:nad,activeProj:id});};
  const selectDraft=id=>{const d=draftList.find(x=>x.id===id);if(!d)return;const nd=drafts[activeProj]?.length?drafts:{...drafts,[activeProj]:draftList};const nad={...activeDrafts,[activeProj]:id};const nl={...lyrics,[activeProj]:d.text||""};setDrafts(nd);setActiveDrafts(nad);setLyrics(nl);doSave({drafts:nd,activeDrafts:nad,lyrics:nl});setTimeout(()=>syncMobileEditor(d.text||""),0);};
  const addDraft=()=>{const base=drafts[activeProj]?.length?drafts[activeProj]:draftList;const next=makeDraft(base.length,curText);const list=[...base,next];const nd={...drafts,[activeProj]:list};const nad={...activeDrafts,[activeProj]:next.id};const nsv=Object.keys(varStore).length?{...sectionVariants,[variantScope(activeProj,next.id)]:cloneVariantStore(varStore)}:sectionVariants;setDrafts(nd);setActiveDrafts(nad);setSectionVariants(nsv);doSave({drafts:nd,activeDrafts:nad,sectionVariants:nsv});};
  const deleteDraft=id=>{const base=drafts[activeProj]?.length?drafts[activeProj]:draftList;if(base.length<=1)return;const removedIdx=base.findIndex(d=>d.id===id);const list=base.filter(d=>d.id!==id);const next=id===activeDraft.id?list[Math.max(0,removedIdx-1)]||list[0]:activeDraft;const nd={...drafts,[activeProj]:list};const nad={...activeDrafts,[activeProj]:next.id};const nl={...lyrics,[activeProj]:next.text||""};const nsv={...sectionVariants};delete nsv[variantScope(activeProj,id)];setDrafts(nd);setActiveDrafts(nad);setLyrics(nl);setSectionVariants(nsv);doSave({drafts:nd,activeDrafts:nad,lyrics:nl,sectionVariants:nsv});};
  ...
  const activateProjectDrag=()=>{const s=projectPointerRef.current;if(!s||s.cancelled)return;s.dragging=true;suppressProjectClickRef.current=true;setLongPressMenu(null);setProjectDrag({id:s.id,target:findProjectDropTarget(s.lastX,s.lastY)});try{document.body.style.overflow="hidden";}catch(err){}};
  const startProjectDrag=(e,id,immediate=false)=>{if(editingName===id)return;clearProjectDragTimer();e.preventDefault();e.stopPropagation();e.currentTarget?.setPointerCapture?.(e.pointerId);projectPointerRef.current={id,x:e.clientX,y:e.clientY,lastX:e.clientX,lastY:e.clientY,dragging:false,cancelled:false};if(immediate)activateProjectDrag();else projectDragTimerRef.current=setTimeout(activateProjectDrag,360);};
  ...
  if(loading||authLoading||cloudLoading)return(<div style={{fontFamily:mf,position:"fixed",inset:0,display:"flex",alignItems:"center",justifyContent:"center",background:"#0a0a0a",color:"#7a7e8e",fontSize:14}}>//LOADING...</div>);
  ...
  return(
    <div style={{fontFamily:ff,position:"fixed",top:0,left:0,right:0,bottom:0,display:"flex",flexDirection:"column",background:"#0a0a0a",color:"#c8ccd8",overflow:"hidden",maxWidth:480,margin:"0 auto"}}>
      ...
      <div style={{padding:"12px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0,borderBottom:"1px solid #1a1a1a"}}>
        ...
        {tab==="editor"&&<button title={barsCollapsed?"ツールバーを表示":"ツールバーをたたむ"} onClick={toggleBars} className="lw-m-chip" style={{...btn,width:34,height:36,borderRadius:10,background:"transparent",border:"1px solid transparent",color:barsCollapsed?"#4a4e5e":"#4af0a0",flexShrink:0,marginLeft:4}}><PanelIcon on={!barsCollapsed} size={17}/></button>}
      ...
      <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column"}}>
        ...
        {tab==="editor"&&(<div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          ...
          <div className="lw-m-bars" data-collapsed={barsCollapsed?"true":"false"}>
          <div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 12px 8px 12px",flexShrink:0,borderBottom:"1px solid #1a1a1a"}}>
            <button data-take-anchor="true" className="lw-m-chip" onClick={e=>{const r=e.currentTarget.getBoundingClientRect();setTakeMenu(takeMenu?null:{left:r.left,top:r.bottom+6,width:Math.max(r.width,150)});}} style={{...btn,flexShrink:0,gap:4,fontSize:11,fontFamily:mf,fontWeight:600,color:"#4af0a0",background:"#4af0a018",border:"1px solid #4af0a040",borderRadius:6,padding:"4px 8px"}}><span>{draftDisplayTitle(activeDraft,draftList.findIndex(d=>d.id===activeDraft.id))}</span>{draftList.length>1&&<span style={{fontSize:9,opacity:.7}}>/{draftList.length}</span>}<ChevronDown size={11} style={{transform:takeMenu?"rotate(180deg)":"none",transition:"transform 200ms var(--lw-motion)"}}/></button>
            ...
            <div className="lw-hscroll" style={{flex:1,minWidth:0,display:"flex",gap:6,alignItems:"center",overflowX:"auto",overflowY:"hidden",whiteSpace:"nowrap",paddingBottom:2}}>
              {sections.map((s,i)=>{const label=s.label.trim();const isActive=caretSection===label;const open=colorPicker?.kind==="section"&&colorPicker.key===s.key;const info=isActive?variantInfo(label):null;const openPicker=e=>setColorPicker(open?null:{kind:"section",key:s.key,label:s.label,anchor:e.currentTarget.getBoundingClientRect()});
                if(!isActive)return <button key={i} className="lw-m-chip" onClick={openPicker} style={{...btn,flexShrink:0,fontSize:11,fontFamily:mf,fontWeight:500,color:s.color,background:s.color+(open?"2c":"18"),border:`1px solid ${s.color}${open?"80":"40"}`,borderRadius:6,padding:"4px 12px"}}>{s.label}</button>;
                // Caret section: expanded chip with its variants inside
                const shortLabel=s.label.length>6?s.label.slice(0,5)+"…":s.label;
                return <div key={i} className="lw-m-chip lw-m-chip-open" style={{display:"inline-flex",alignItems:"stretch",flexShrink:0,background:s.color+"22",border:`1px solid ${s.color}90`,borderRadius:6,overflow:"hidden"}}>
                  <button className="lw-m-chip" onClick={openPicker} title="タップで色変更" style={{...btn,fontSize:11,fontFamily:mf,fontWeight:600,color:s.color,padding:"4px 10px",borderRadius:0}}>{info?shortLabel:s.label}</button>
                  <div style={{width:1,background:s.color+"50"}}/>
                  {info&&info.entry.variants.map((v,k)=>{const active=v.id===info.entry.activeId;return <button key={v.id} className="lw-m-chip" onClick={()=>!active&&applyVariant(label,v.id)} style={{...btn,gap:3,fontSize:11,fontFamily:mf,fontWeight:700,color:active?"#0a0a0d":s.color,background:active?s.color:"transparent",padding:"4px 9px",borderRadius:0}}><span>{variantLabel(k)}</span>{active&&<span onClick={e=>{e.stopPropagation();deleteVariant(label,v.id);}} style={{display:"grid",placeItems:"center",width:12,height:12,opacity:.8}}><XIcon size={9}/></span>}</button>;})}
                  <button className="lw-m-chip" title="今の内容を複製して別パターンを作る" onClick={()=>addVariant(label)} style={{...btn,gap:3,fontSize:10,fontFamily:mf,color:s.color,padding:"4px 8px",borderRadius:0,opacity:.85}}><Plus size={10}/>{info?"":"候補"}</button>
                </div>;})}
              {sections.length===0&&<span style={{fontSize:10,color:"#4a4e5e"}}>[Verse] のように書くとセクションになります</span>}
            </div>
            <button className="lw-m-chip" title="文字色" onClick={e=>setColorPicker(colorPicker?.kind==="text"?null:{kind:"text",anchor:e.currentTarget.getBoundingClientRect()})} style={{...btn,flexShrink:0,padding:"4px 6px",borderRadius:6,border:"1px solid "+(colorPicker?.kind==="text"?"rgba(74,240,160,0.3)":"#3a3a4a"),color:curTheme.text}}><TextColorIcon size={13}/></button>
            <button className="lw-m-chip" title="背景色" onClick={e=>setColorPicker(colorPicker?.kind==="bg"?null:{kind:"bg",anchor:e.currentTarget.getBoundingClientRect()})} style={{...btn,flexShrink:0,padding:"4px 6px",borderRadius:6,border:"1px solid "+(colorPicker?.kind==="bg"?"rgba(74,240,160,0.3)":"#3a3a4a"),color:curTheme.bg===THEME_DEFAULT.bg?"#7a7e8e":curTheme.bg}}><FillIcon size={13}/></button>
          </div>
          </div>
          ...
          {/* Take dropdown (anchored to the chip, same width) */}
          {takeMenu&&(<div ref={takeMenuRef} className="lw-motion-dropdown" style={{position:"fixed",left:Math.max(8,Math.min(takeMenu.left,window.innerWidth-takeMenu.width-8)),top:takeMenu.top,width:takeMenu.width,zIndex:2300,background:"#111116",border:"1px solid #3a3a4a",borderRadius:10,boxShadow:"0 16px 36px rgba(0,0,0,0.5)",padding:6,display:"flex",flexDirection:"column",gap:4,maxHeight:"50vh",overflowY:"auto"}}>
            {draftList.map((d,i)=>{const active=d.id===activeDraft.id;return <div key={d.id} style={{display:"flex",alignItems:"center",gap:4}}>
              <button className="lw-m-chip" onClick={()=>{selectDraft(d.id);setTakeMenu(null);}} style={{...btn,flex:1,minWidth:0,justifyContent:"flex-start",padding:"8px 10px",borderRadius:7,fontSize:11,fontFamily:mf,fontWeight:active?600:500,color:active?"#4af0a0":"#c8ccd8",background:active?"rgba(74,240,160,0.1)":"transparent",border:"1px solid "+(active?"rgba(74,240,160,0.3)":"transparent")}}><span style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{draftDisplayTitle(d,i)}</span></button>
              {draftList.length>1&&<button className="lw-m-chip" title="この Take を削除" onClick={()=>deleteDraft(d.id)} style={{...btn,width:28,height:30,borderRadius:7,color:"#4a4e5e",flexShrink:0}}><XIcon size={11}/></button>}
            </div>;})}
          ...
          <div style={{flex:1,overflow:"hidden",position:"relative",display:"flex",background:curTheme.bg}}>
            ...
            <div ref={mobileGutterRef} style={{flexShrink:0,overflow:"hidden",userSelect:"none",background:"transparent"}}>
              <div style={{display:"flex",paddingTop:16,paddingBottom:16,transform:`translateY(${-mobileScrollTop}px)`,willChange:"transform"}}>
                <div style={{width:3,flexShrink:0}}>{mobileLines.map((l,i)=>(<div key={i} style={{height:mobileLineHeight,background:mobileSecMap[i]||"transparent",opacity:getSecLabel(l)?1:0.4,boxSizing:"border-box"}}/>))}</div>
                <div style={{width:42}}>{mobileLines.map((l,i)=>{const label=getSecLabel(l),isSection=!!label,isActive=i===mobileCurrentLine,sc=getSecColor(l,sectionColors),iD=!!lineDrag&&i>=lineDrag.from&&i<lineDrag.end;const dragStyle={touchAction:"none",cursor:lineDrag?"grabbing":"grab",background:iD?"rgba(74,240,160,0.12)":"transparent",borderRadius:4};if(isSection){mobileSectionLine=0;return <div key={i} onPointerDown={onGutterPointerDown(i)} style={{height:mobileLineHeight,lineHeight:mobileLineHeight+"px",fontSize:9,fontFamily:mf,textAlign:"right",paddingRight:10,color:sc,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",boxSizing:"border-box",...dragStyle}}>{label}</div>;}mobileSectionLine+=1;return <div key={i} onPointerDown={onGutterPointerDown(i)} style={{height:mobileLineHeight,lineHeight:mobileLineHeight+"px",fontSize:11,fontFamily:mf,textAlign:"right",paddingRight:10,color:iD?"#4af0a0":isActive?"#7a7e8e":"#3a3a4a",fontWeight:400,boxSizing:"border-box",...dragStyle}}>{mobileSectionLine}</div>;})}</div>
              </div>
            </div>
            <textarea ref={mobileTextRef} value={curText} onChange={e=>{setCurText(e.target.value);setTimeout(()=>syncMobileEditor(e.target.value),0);}} onFocus={()=>setTimeout(syncMobileEditor,0)} onBlur={()=>setMobileCaret(c=>({...c,visible:false}))} onScroll={syncMobileEditor} onClick={syncMobileEditor} onKeyUp={syncMobileEditor} onSelect={e=>{syncMobileEditor();checkSelection(e);}} onTouchEnd={()=>setTimeout(()=>{syncMobileEditor();checkSelection();},200)} spellCheck={false} wrap="off" style={{flex:1,minWidth:0,height:"100%",fontFamily:ff,fontSize:16,lineHeight:mobileLineHeight+"px",letterSpacing:"0.02em",caretColor:"#4af0a0",background:"transparent",color:curTheme.text,border:"none",outline:"none",resize:"none",padding:"16px 16px 16px 8px",boxSizing:"border-box",overflow:"auto",whiteSpace:"pre"}}/>
          </div>
          {/* Selection toolbar */}
