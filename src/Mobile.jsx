...
export default function MobileApp(){
  ...
  const[editorTheme,setEditorTheme]=useState(null); // PC-only; passed through so mobile saves keep it // PC-only feature; carried through so mobile saves don't drop it
  ...
  const addDraft=()=>{const base=drafts[activeProj]?.length?drafts[activeProj]:draftList;const next=makeDraft(base.length,curText);const list=[...base,next];const nd={...drafts,[activeProj]:list};const nad={...activeDrafts,[activeProj]:next.id};setDrafts(nd);setActiveDrafts(nad);doSave({drafts:nd,activeDrafts:nad});};
  const deleteDraft=id=>{const base=drafts[activeProj]?.length?drafts[activeProj]:draftList;if(base.length<=1)return;const removedIdx=base.findIndex(d=>d.id===id);const list=base.filter(d=>d.id!==id);const next=id===activeDraft.id?list[Math.max(0,removedIdx-1)]||list[0]:activeDraft;const nd={...drafts,[activeProj]:list};const nad={...activeDrafts,[activeProj]:next.id};const nl={...lyrics,[activeProj]:next.text||""};setDrafts(nd);setActiveDrafts(nad);setLyrics(nl);doSave({drafts:nd,activeDrafts:nad,lyrics:nl});};
    ...
    <div style={{fontFamily:ff,position:"fixed",top:0,left:0,right:0,bottom:0,display:"flex",flexDirection:"column",background:"#0a0a0a",color:"#c8ccd8",overflow:"hidden",maxWidth:480,margin:"0 auto"}}>
      ...
      <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column"}}>
        ...
        {tab==="editor"&&(<div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          {sections.length>0&&(<div style={{padding:"10px 16px",display:"flex",gap:6,flexWrap:"wrap",flexShrink:0,borderBottom:"1px solid #1a1a1a"}}>{sections.map((s,i)=>(<label key={i} title="クリックで色変更" style={{display:"inline-flex",alignItems:"center",gap:6,fontSize:11,fontFamily:mf,fontWeight:500,color:s.color,background:s.color+"18",border:`1px solid ${s.color}40`,borderRadius:6,padding:"4px 12px",cursor:"pointer"}}><input type="color" value={s.color} onChange={e=>updateSectionColor(s.key,e.target.value)} style={{width:13,height:13,padding:0,border:"none",background:"transparent"}}/>{s.label}</label>))}</div>)}
          ...
          <div style={{flex:1,overflow:"hidden",position:"relative",display:"flex"}}>
            <div ref={mobileGutterRef} style={{flexShrink:0,overflow:"hidden",userSelect:"none",background:"#0a0a0a"}}>
              <div style={{display:"flex",paddingTop:16,paddingBottom:16,transform:`translateY(${-mobileScrollTop}px)`,willChange:"transform"}}>
                ...
                <div style={{width:42}}>{mobileLines.map((l,i)=>{const label=getSecLabel(l),isSection=!!label,isActive=i===mobileCurrentLine,sc=getSecColor(l,sectionColors);if(isSection){mobileSectionLine=0;return <div key={i} style={{height:mobileLineHeight,lineHeight:mobileLineHeight+"px",fontSize:9,fontFamily:mf,textAlign:"right",paddingRight:10,color:sc,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",boxSizing:"border-box"}}>{label}</div>;}mobileSectionLine+=1;return <div key={i} style={{height:mobileLineHeight,lineHeight:mobileLineHeight+"px",fontSize:11,fontFamily:mf,textAlign:"right",paddingRight:10,color:isActive?"#7a7e8e":"#3a3a4a",fontWeight:400,boxSizing:"border-box"}}>{mobileSectionLine}</div>;})}</div>
              </div>
            </div>
