        ...
        {tab==="editor"&&(<div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:6,padding:"8px 12px 8px 16px",flexShrink:0,borderBottom:"1px solid #1a1a1a"}}>
            <div className="lw-hscroll" style={{flex:1,minWidth:0,display:"flex",gap:6,alignItems:"center",overflowX:"auto",overflowY:"hidden",whiteSpace:"nowrap",paddingBottom:2}}>
              {sections.map((s,i)=>{const isActive=caretSection===s.label.trim();const open=colorPicker?.kind==="section"&&colorPicker.key===s.key;return <button key={i} className="lw-m-chip" onClick={e=>setColorPicker(open?null:{kind:"section",key:s.key,label:s.label,anchor:e.currentTarget.getBoundingClientRect()})} style={{...btn,flexShrink:0,fontSize:11,fontFamily:mf,fontWeight:500,color:s.color,background:s.color+(isActive||open?"2c":"18"),border:`1px solid ${s.color}${isActive||open?"80":"40"}`,borderRadius:6,padding:"4px 12px"}}>{s.label}</button>;})}
              {sections.length===0&&<span style={{fontSize:10,color:"#4a4e5e"}}>[Verse] のように書くとセクションになります</span>}
            </div>
            <button className="lw-m-chip" title="文字色" onClick={e=>setColorPicker(colorPicker?.kind==="text"?null:{kind:"text",anchor:e.currentTarget.getBoundingClientRect()})} style={{...btn,flexShrink:0,padding:"4px 7px",borderRadius:6,border:"1px solid "+(colorPicker?.kind==="text"?"rgba(74,240,160,0.3)":"#3a3a4a"),color:curTheme.text}}><TextColorIcon size={13}/></button>
            <button className="lw-m-chip" title="背景色" onClick={e=>setColorPicker(colorPicker?.kind==="bg"?null:{kind:"bg",anchor:e.currentTarget.getBoundingClientRect()})} style={{...btn,flexShrink:0,padding:"4px 7px",borderRadius:6,border:"1px solid "+(colorPicker?.kind==="bg"?"rgba(74,240,160,0.3)":"#3a3a4a"),color:curTheme.bg===THEME_DEFAULT.bg?"#7a7e8e":curTheme.bg}}><FillIcon size={13}/></button>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 12px 8px 16px",flexShrink:0,borderBottom:"1px solid #1a1a1a"}}>
            ...
            <div className="lw-hscroll" style={{flex:1,minWidth:0,display:"flex",gap:6,alignItems:"center",overflowX:"auto",overflowY:"hidden",whiteSpace:"nowrap",paddingBottom:2}}>
              {caretSection?(()=>{const info=variantInfo(caretSection);const color=getSecColor("["+caretSection+"]",sectionColors);return (<>
                ...
                {info&&info.entry.variants.map((v,i)=>{const active=v.id===info.entry.activeId;return <button key={v.id} className="lw-m-chip" onClick={()=>!active&&applyVariant(caretSection,v.id)} style={{...btn,flexShrink:0,gap:5,fontSize:11,fontFamily:mf,fontWeight:600,color:active?color:"#7a7e8e",background:(active?color:"#7a7e8e")+"18",border:`1px solid ${active?color:"#7a7e8e"}40`,borderRadius:6,padding:"4px 10px"}}><span>{variantLabel(i)}</span>{active&&<span onClick={e=>{e.stopPropagation();deleteVariant(caretSection,v.id);}} style={{display:"grid",placeItems:"center",width:13,height:13,opacity:.75}}><XIcon size={9}/></span>}</button>;})}
                <button className="lw-m-chip" title="今の内容を複製して別パターンを作る" onClick={()=>addVariant(caretSection)} style={{...btn,flexShrink:0,gap:4,fontSize:11,fontFamily:mf,fontWeight:500,color:"#7a7e8e",background:"#7a7e8e18",border:"1px solid #7a7e8e40",borderRadius:6,padding:"4px 9px"}}><Plus size={10}/>{info?"":"候補"}</button>
