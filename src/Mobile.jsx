            ...
            <div className="lw-hscroll" style={{flex:1,minWidth:0,display:"flex",gap:6,alignItems:"center",overflowX:"auto",overflowY:"hidden",whiteSpace:"nowrap",paddingBottom:2}}>
              {sections.filter((s,i,arr)=>arr.findIndex(x=>x.label.trim()===s.label.trim())===i).map((s,i)=>{const label=s.label.trim();const isActive=caretSection===label;const open=colorPicker?.kind==="section"&&colorPicker.key===s.key;const info=isActive?variantInfo(label):null;const openPicker=e=>setColorPicker(open?null:{kind:"section",key:s.key,label:s.label,anchor:e.currentTarget.getBoundingClientRect()});
                ...
                return <div key={i} className="lw-m-chip lw-m-chip-open" style={{display:"inline-flex",alignItems:"stretch",flexShrink:0,background:s.color+"22",border:`1px solid ${s.color}90`,borderRadius:6,overflow:"hidden"}}>
                  ...
                  <button className="lw-m-chip" title="今の内容を複製して別パターンを作る" onClick={()=>addVariant(label)} style={{...btn,gap:3,fontSize:10,fontFamily:mf,color:s.color,padding:"4px 8px",borderRadius:0,opacity:.85}}><Plus size={10}/></button>
                </div>;})}
              {sections.length===0&&<span style={{fontSize:10,color:"#4a4e5e"}}>[Verse] のように書くとセクションになります</span>}
              {sections.length>0&&!caretSection&&<span style={{fontSize:10,color:"#4a4e5e"}}>セクション内にカーソルを置くと候補を操作できます</span>}
            </div>
